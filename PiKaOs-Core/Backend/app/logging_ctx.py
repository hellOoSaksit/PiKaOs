...
Every worker log line carries the run it belongs to — run_id / parent_run_id / quest_id —
...
Center Server (modularity.md), to tell which run/quest a noisy log line came from.
"""
...
_FIELDS = ("run_id", "parent_run_id", "quest_id", "agent_id")

_run_ctx: contextvars.ContextVar[dict] = contextvars.ContextVar("pikaos_run_ctx", default={})

LOG_FORMAT = (
    "%(asctime)s %(levelname)s %(name)s "
    "[run=%(run_id)s parent=%(parent_run_id)s quest=%(quest_id)s agent=%(agent_id)s] %(message)s"
)

...
def main() -> None:
    ...
    OUT_COMPOSE.write_text(
        "# GENERATED by Backend/scripts/render_compose.py — DO NOT EDIT BY HAND.\n"
        "# Edit deploy/docker-compose.dev.yml (kernel base) or a plugin's compose.fragment.yml\n"
        "# instead, then re-run this script (start.sh / start.bat do it automatically).\n"
        + result.stdout,
        encoding="utf-8",
    )
    tool_ids = sorted(p.parent.name for p in fragments)
    print(f"[render_compose] enabled tools: {', '.join(tool_ids) or '(none)'} -> {OUT_COMPOSE}")

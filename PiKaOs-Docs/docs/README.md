...
| [`repo-split-plan.md`](architecture/repo-split-plan.md) | **Phase-5 repo-split design (not yet executed)** — PiKaOs-App composition root + features as own repos; the cross-repo dependency/deploy decision (packages+entry-points vs workspace path deps) with a recommendation + reversible steps | before splitting Core/App/Plugin into separate deployables |

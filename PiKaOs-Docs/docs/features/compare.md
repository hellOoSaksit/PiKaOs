---
title: Compare — UAT vs Production compare feature
type: feature
status: built
keywords: [compare, uat, production, sitemap coverage, deep diff, swap_origin, streamed batch, outbound http, plugin]
related: [./compare-hardening.md, ./checklist-audit.md, ../plugin/compare/README.md, ../pikaos-dev-rules.md]
summary: >
  Source of truth for the compare engine behavior (sitemap coverage + deep page diff over
  user-supplied URLs). Read before changing anything in the compare path.
updated: 2026-06-20
---
...
[pikaos-dev-rules.md](../pikaos-dev-rules.md) §2.6 to keep that file ≤300 lines. Read this before changing
anything in the compare path.

> **Also shipped as a plugin app** — this same engine lifted into its own no-login Docker app
> (ports 5174/8001). Plugin-specific deltas / status / errors / merge-back:
> [`../plugin/compare/`](../plugin/compare/README.md). This file stays the source of truth
> for the **engine behavior**; keep both in sync when you change the compare path.
...
Files:
- Backend: [`routers/compare.py`](../../../PiKaOs-Core/Backend/app/routers/compare.py) → [`services/compare_service.py`](../../../PiKaOs-Core/Backend/app/services/compare_service.py) · [`services/sitemap.py`](../../../PiKaOs-Core/Backend/app/services/sitemap.py) · [`services/content.py`](../../../PiKaOs-Core/Backend/app/services/content.py) · [`services/net_guard.py`](../../../PiKaOs-Core/Backend/app/services/net_guard.py) (SSRF guard — see [compare-hardening.md](compare-hardening.md))
- Schemas: [`schemas.py`](../../../PiKaOs-Core/Backend/app/schemas.py) (`CompareIn`/`CompareOut`/`UrlCheck`/`DeepResult`/`DeepPair`/`DeepBatchIn`/`DeepBatchOut` · streamed coverage: `CoveragePlanIn`/`CoveragePlanOut`/`CoveragePair`/`CoverageBatchIn`/`CoverageBatchOut`)
- Settings: [`config.py`](../../../PiKaOs-Core/Backend/app/config.py) (`compare_*`) · Tests: [`tests/test_compare.py`](../../../PiKaOs-Core/Backend/tests/test_compare.py)
- Frontend: [`screens-compare.jsx`](../../../PiKaOs-Core/Frontend/src/screens/screens-compare.jsx) · [`lib/api.js`](../../../PiKaOs-Core/Frontend/src/lib/api.js) (`coveragePlan`/`coverageBatch`/`compareDeep`)
...
- **Two pages (direct)** — enter **two exact page URLs** (even unrelated sites, e.g.
  ...
  would find zero matches → no detail). `runPair()` in [screens-compare.jsx](../../../PiKaOs-Core/Frontend/src/screens/screens-compare.jsx).
...
([fx.css](../../../PiKaOs-Core/Frontend/src/styles/fx.css)) + a progress bar. A batch error marks its rows
...
`_run_cancellable(request, coro)` ([routers/compare.py](../../../PiKaOs-Core/Backend/app/routers/compare.py))
...
- **Deep detail = DIFFERENCES ONLY** (`DeepDetail`, click a row). A field/section appears **only
  ...
  Expand is animated ([fx.css](../../../PiKaOs-Core/Frontend/src/styles/fx.css) `.cmp-grow`/`.cmp-reveal`/`.cmp-detail`/`.cmp-chev`):
...
The store lives in its own data module [`data/compare-sites.jsx`](../../../PiKaOs-Core/Frontend/src/data/compare-sites.jsx)
...
## 5. Settings ([`config.py`](../../../PiKaOs-Core/Backend/app/config.py))
...
## 6. Gotchas
- **Proxy timeout:** the Vite dev proxy `/api` timeout is **180s** ([`vite.config.js`](../../../PiKaOs-Core/Frontend/vite.config.js)) —
...
- **Completion toasts:** the app is wrapped in `ToastProvider` ([App.jsx](../../../PiKaOs-Core/Frontend/src/App.jsx));
...
## 7. Tests
[`tests/test_compare.py`](../../../PiKaOs-Core/Backend/tests/test_compare.py): pure logic (swap/parse/classify),

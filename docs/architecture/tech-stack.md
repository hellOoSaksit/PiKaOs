...
|---|---|---|
| db | `postgres:16-alpine` | Postgres 16 (pgvector removed — unused; re-add as extension in phase E if RAG needs it), healthcheck `pg_isready` |
...
| worker | build `./Backend` | `arq app.worker.WorkerSettings` (engine jobs out-of-process, B2) |
| frontend | build `./Frontend` | `node:22-alpine` → Vite dev server :5173; proxy `/api`+`/ws` → `backend:8000`; hot reload (bind mount + `VITE_POLL`) |

ทั้งสแตกอยู่ใน Docker (รวม frontend) — `start.bat` = `docker compose up -d --build` + เปิด browser, ดู log ใน Docker (กติกา CLAUDE.md §0).
...
Windows host (start.bat)
 ...
 └─ Docker
     ├─ backend (FastAPI, uvicorn) :8000 ─┬─▶ db (postgres:16) :5432

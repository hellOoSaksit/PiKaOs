...
| B3 | ✅ **เสร็จ (2026-06-16)** `services/agent_runner.run` loop (worker job `agent_run`) + `repositories/runs.py` (SQL ทั้งหมด รวม atomic `reserve_quota` UPDATE…RETURNING) + config ใหม่ (`run_max_steps`/`run_llm_step_timeout_s`/`run_tool_step_timeout_s`/`run_max_wallclock_s`) + cancel flag (`redis_client.request_run_cancel`/`is_run_cancelled`). 2-phase tool (pending→done, `idempotency_key="{run_id}:{seq}"`) · resume by effect class (read/idempotent_write→rerun, side_effect/unknown→`waiting_input`) · per-step `asyncio.wait_for` · LLM provider+tool registry **injected** (`set_engine_runtime`; stub=B4). 12 tests ใน `test_agent_runner.py` (pure helpers + loop ด้วย in-memory fake repo) — **93 passed**. |
| B4 | ✅ **เสร็จ (2026-06-16)** `services/engine_stubs.py`: **StubLLMProvider** (script ใน `@@stub@@`+JSON ใน seed message → ตอบตาม turn) + **StubToolRegistry** 3 ตัวครบ effect class (`echo`=read · `upsert`=idempotent_write ON CONFLICT DO NOTHING · `record`=side_effect plain INSERT). Sink = `stub_tool_writes` (migration `0005` + ORM `StubToolWrite` + `repositories/stub_tools.py`). ผูก `set_engine_runtime` ตอน `worker.startup`. + roll-up `runs.tokens_used` (repo `add_run_tokens`). 8 tests (`test_engine_stubs.py`) + smoke ผ่าน worker จริง: enqueue→`llm/tool/llm`=done, `run_steps`=[(0,llm,6),(1,tool,0),(2,llm,3)], stub write 1 แถว key `{run_id}:1`, `tokens_used=9`. **101 passed.** |
| B5 | ✅ **เสร็จ (2026-06-16)** `services/events.py` (publish per-step + per-run event → Redis `quest:<id>`, best-effort, payload cap 16KB, `serialize_step` ใช้ร่วม) — runner emit ทุก step/status (running/step llm+tool 2-phase/done/failed/cancelled/waiting_input). `services/quest_service.py` (authz `can_view` = owner/dept member/admin · `snapshot` runs+steps · `backfill` มี cross-quest guard) + `repositories/quests.py`. `routers/ws.py` ปลด stub: subscribe→authz→snapshot, frame `backfill`. 7 tests (`test_quest_stream.py`) + smoke pubsub จริง: `run:running→llm→tool(pending)→tool(done)→llm→run:done`. **108 passed.** |
| B6 | ✅ **เสร็จ (2026-06-16)** `test_engine_resume.py` — รัน `agent_runner.run` ตรงบน Postgres จริง (local-engine + `db_factory` inject), จำลอง worker ตายกลาง step ด้วย `_Crash(BaseException)` ที่ลอด `except Exception`. 4 acceptance gates ผ่าน: (1) ฆ่ากลาง side_effect → resume `waiting_input` เขียนแถวเดียว ไม่ยิงซ้ำ (2) ฆ่ากลาง LLM → resume สร้าง conversation จาก run_steps ต่อได้ ไม่มี step ซ้ำ (3) quota พอดีเส้น → run ที่สอง `quota_exceeded`, `used == Σ run_steps.tokens` (4) snapshot กู้ timeline ครบ. **112 passed.** |
| B7 | ✅ **เสร็จ (2026-06-16)** Structured logging — `logging_ctx.py` (contextvars + `RunContextFilter` stamp ทุก record ด้วย `run_id`/`parent_run_id`/`quest_id`/`agent_id`, default `-` เมื่อไม่ผูก) · `bind_run`/`reset_run` ใน `run_job` (ผูก/คืนต่อ job ไม่รั่วข้าม run) + enrich ใน `run()` · `configure_worker_logging` (handler+formatter+filter scoped `pikaos.*`, propagate=False ไม่ชน arq) · INFO `run started`/`run done`. 5 tests + smoke worker จริง: `INFO pikaos.engine [run=… quest=…] run done (steps=3, tokens=7)`. **117 passed.** |

**เฟส B ปิดครบ ✅ (B1–B7)** — engine core: schema · arq worker · agent loop (2-phase/resume/quota/timeout) · stub LLM+tools · live worklog stream · resume/crash integration · structured logging. ต่อ provider จริง = เฟส C.
...
> **markdown = source of truth · pgvector = cache ทิ้ง/สร้างใหม่ได้** (เปิดเมื่อถึงเกณฑ์ §3 ของ doc).
> ตัวเก็บ markdown (ชั้น 2) ทำได้เลย; E1–E4 คือชั้น vector เมื่อจำเป็น. กฎเหล็ก: rebuild ทางเดียว `markdown → vector`.

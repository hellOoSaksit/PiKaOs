...
- ✅ **A1 RBAC server-side เสร็จแล้ว (2026-06-15)** → migration `0002_rbac` (4 ตาราง) · `repositories/rbac.py` ·
  ...
  **ต้อง restart backend container** ให้ migration+seed รัน. **เฟส A ปิดแล้ว** — เหลือเฉพาะ A2 ส่วนเฟส B (per-quest authz + run_steps backfill)
  ...
  migration ล่าสุด `0004_engine` (B1 ✅); backend tests **93 เขียว**; CI `.github/workflows/ci.yml` รันจริงตอน push.
> **[2026-06-16] เฟส B เริ่ม — B1 ✅:** `0004_engine` สร้าง 10 ตาราง engine (`departments`/`user_departments` m:n · `rooms`/`agents`/`quests`/`runs`/`subtasks`/`run_steps`/`tools_config`/`notifications`) + `documents.department_id` · FK/cascade/UNIQUE ตาม risk-mitigation §4.4 · ORM models ใน `models.py` · schema-only (seed แผนก + CRUD = เฟส D). · **B2 ✅** `worker` service ใน compose + `app/worker.py` (arq 0.28, ping job; enqueue→pong ยืนยันแล้ว). · **B3 ✅ (2026-06-16)** `services/agent_runner.run` loop + worker job `agent_run` + `repositories/runs.py` (atomic `reserve_quota`) + config timeouts/max_steps + cancel flag ใน `redis_client`. 2-phase tool (`idempotency_key="{run_id}:{seq}"`) · resume by effect class (read/idempotent_write→rerun, side_effect/unknown→`waiting_input`) · per-step `asyncio.wait_for` · LLM provider+tool registry **inject** ผ่าน `set_engine_runtime` (stub จริง=B4). worker boot ขึ้น `ping, agent_run`. · **B4 ✅ (2026-06-16)** `services/engine_stubs.py` (StubLLMProvider อ่าน script `@@stub@@`+JSON จาก seed message · StubToolRegistry 3 ตัว: `echo`=read/`upsert`=idempotent_write/`record`=side_effect) → sink `stub_tool_writes` (migration `0005` + `repositories/stub_tools.py`) → ผูก `set_engine_runtime` ที่ `worker.startup` แล้ว · roll-up `runs.tokens_used` (`add_run_tokens`). Smoke ผ่าน worker จริง: run=done, steps=llm/tool(2-phase)/llm, side-effect เขียน 1 แถว, `tokens_used=9`. **101 tests เขียว.** · **B5 ✅ (2026-06-16)** `services/events.py` (per-step+per-run event → Redis `quest:<id>`, best-effort, cap 16KB) — runner emit ครบทุก step/status. `services/quest_service.py` (authz `can_view` owner/dept/admin · `snapshot` · `backfill`+cross-quest guard) + `repositories/quests.py`. `routers/ws.py` ปลด stub A2: subscribe→authz→snapshot + frame `backfill`. Smoke pubsub จริง: `run:running→llm→tool(pending)→tool(done)→llm→run:done`. **108 tests เขียว.** · **B6 ✅ (2026-06-16)** `test_engine_resume.py` — integration บน Postgres จริง (local-engine + `db_factory` inject, จำลอง crash ด้วย `_Crash(BaseException)` ที่ลอด `except Exception`). 4 acceptance gates ผ่านครบ: ฆ่ากลาง side_effect → `waiting_input` เขียนแถวเดียว · ฆ่ากลาง LLM → resume ต่อ conversation เดิม ไม่ซ้ำ · quota พอดีเส้น `used==Σ tokens` + `quota_exceeded` · snapshot กู้ timeline. **112 tests เขียว.** **ถัดไป B7 (structured logging: ทุกบรรทัดใน worker มี `run_id`/`parent_run_id`/`quest_id` — ปิดเฟส B) → แล้วเข้าเฟส C (HERMES + LLM provider จริง C1–C6) หรือ D (ย้าย FE→BE) ขนานได้.**

"""lxml + httpx crawler adapter (implements domain.ports.Crawler).

Extracts candidate "page terms" — nav/menu links, headings, <title>,
breadcrumbs, and sitemap.xml <loc> entries — dropping popup/consent containers.
"""
...
from ..domain.entities import PageTerm
...
class LxmlCrawler:
    def fetch_and_extract(self, url: str, bypass_popup: bool) -> tuple[str, list[PageTerm]]:
        final_url, html_text = self._fetch(url)
        return final_url, self._extract(final_url, html_text, bypass_popup)
    ...
    def _try_sitemap(self, base_url: str) -> list[PageTerm]:
        out: list[PageTerm] = []
    ...
    def _extract(self, final_url: str, html_text: str, bypass_popup: bool) -> list[PageTerm]:
        ...
        terms.extend(self._try_sitemap(final_url))
